...
async def update_comanda(id: int, comanda_data: ComandaUpdate, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
    try:
        ...
        # se não encontrou, tenta pelo número da comanda
        if not comanda:
            result = await db.execute(
                select(ComandaDB).where(ComandaDB.comanda == id)
            )
            comanda = result.scalar_one_or_none()
...
Depends(get_current_active_user)):
    try:
       # tenta buscar pelo ID
        ...
        if not comanda:
            result = await db.execute(
                select(ComandaDB).where(ComandaDB.comanda == comanda_id)
            )
            comanda = result.scalar_one_or_none()
...
async def delete_comanda(
    ...
    try:
        ...
        # 🔹 Se não achou, busca pelo número
        if not comanda:
            result = await db.execute(
                select(ComandaDB).where(ComandaDB.comanda == id)
            )
        ...
        if produtos_count > 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Não é possível excluir comanda com {produtos_count} produtos vinculados"
            )
